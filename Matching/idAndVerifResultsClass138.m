%IDANDVERIFRESULTSCLASS138 Returns Recognition Rate and False Alarm Rate
% for identification performance, and Verification Rate and False Alarm
% Rate for verification performance for a face recognition algorithm based 
% on thermal signatures enhanced with 138 NSS features.
% 
...
function [idRecognitionRate, idFalseAlarmRate, verificationRate, veFalseAlarmRate] = ...
    idAndVerifResultsClass138(TestDir,Database,RecognitionThreshold)
...
for i = 1:n
    ...
    if strcmp(name(end-4:end),'.tiff') || strcmp(name(end-3:end),'.bmp') || strcmp(name(end-3:end),'.jpg')
        ...
        Scores = ThermalSimilarityDatabaseClass138(name,Database,1);
...
if ~is_genuine
    idRecognitionRate = 0;
    verificationRate = 0;
    save('ImpScores_Class','TS_Imp_Measures');
end

if ~is_impostor
    idFalseAlarmRate = 0;
    veFalseAlarmRate = 0;
    save('GenScores_Class','TS_Gen_Measures','SubjectIndex');
end
